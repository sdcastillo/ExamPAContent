...
levels(df$weather)[levels(df$weather) == "OLDLEVEL1"] <- "NEWLEVEL"
levels(df$weather)[levels(df$weather) == "OLDLEVEL2"] <- "NEWLEVEL"
...
df$hour_tree <- df$hour_X
df$hour_glm <- df$hour_X
...
df$temperature_new <- df$temperature_X
...
glm.loglink2 <- glm(pedestrians ~ . - hour_tree, family = DISTRIBUTION(link = "log"), data = data_train)
...
ggplot(data_all, aes(x = CONTINUOUS, y = pedestrians, color = FACTOR)) +
...
glm.interact <- glm(formula = pedestrians ~ . + VARIABLE1*VARIABLE2 - hour_tree, family = DISTRIBUTION(link = "log"), data = data_train)
...
glm.none <- glm(pedestrians ~ 1, data = data_train, family = DISTRIBUTION(link = "log"))
