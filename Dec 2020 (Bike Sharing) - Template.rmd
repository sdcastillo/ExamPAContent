---
title: "December 7, 2020 Exam PA Rmd file"
---
...
levels(data.all$VARIABLE) <- c("LEVEL", "LEVEL", "LEVEL")
...
vars <- c("VARIABLE", "VARIABLE", "VARIABLE")
...
ggplot(data.all, aes(x = VARIABLE1, y = VARIABLE2, color = VARIABLE3)) +
  geom_point(shape = 1)
```
...
data.clustered$cluster <- as.factor(km_$cluster)
...
library(pdp)
partial(boost.tree1, n.trees = 1000, pred.var = "VARIABLE", plot = TRUE)
partial(boost.tree1, n.trees = 1000, pred.var = "VARIABLE", plot = TRUE)
```
...
glm.interaction <- glm(bikes_per_hour ~ . + VARIABLE1 * VARIABLE2, family = poisson(link = "log"), data = data.train)
